...
int memory_test();
int zlib_test(uint8_t* data, int64_t data_size);
int winapi_test();
int win_test(uint8_t* data, int64_t data_size);
...
//	ff.open(file_path1,nullptr,nullptr);
	...
	timer time;
	int countB=0;
	while (detect_active_read_frame(ff->read(&frame)) && time.elapsed()<10000) {
		count++;
		if (frame != nullptr) {
