...
public:
	...
	void set_crop(int crop_x,int crop_y,int crop_w,int crop_h) {
		...
		//video_filter_params v_par;
		v_par.input_crop.lt.x = crop_w;
		v_par.input_crop.lt.y = crop_h;
...
void writer4_set_capture(int64_t capture_ptr) {
	writer4_ctx.write(capture_ptr);
}

void writer4_set_crop(int crop_x, int crop_y, int crop_w, int crop_h) {
	writer4_ctx.set_crop(crop_x,crop_y,crop_w,crop_h);
}
...
void test_simple_recode(AVCodecContext* video_dec_ctx, AVCodecContext* audio_dec_ctx) {
	const char* file_path = "C:\\Users\\Videos\\Test\\test_crop.mp4";
	//get_codec_supported_pixformat(avcodec_find_encoder_by_name("jpeg2000"));
	//const char* file_path = "C:\\Users\\MaMaM\\source\\repos\\f4mthird\\f4mthird\\source\\test.mp4";
	writer_params wp;
	wp.video_encoder_name = "libopenh264";
