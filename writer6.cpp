...
public:
	...
	AVFrame* get_frame() {
		if (first && video.queue.size() && audio.queue.size()) {
			...
			while (0 < compare_pts(video.queue.front(), audio.queue.front())) {
				AVFrame* tmp = audio.queue.front();
				audio.queue.pop();
				av_frame_free(&tmp);
...
public:
	...
	void write(AVMediaType type, AVFrame* frame) {
		stream_ctx& stream_ctx = (type == AVMEDIA_TYPE_VIDEO ? video_stream : audio_stream);
		AVCodecContext* enc_ctx = stream_ctx.enc_ctx;
		...
		while (avcodec_receive_packet(enc_ctx, read_packet) == 0) {
			...
			stream_ctx.latest_pts = (std::max)(stream_ctx.latest_pts,read_packet->pts);
	...
	void close() {
		write(AVMediaType::AVMEDIA_TYPE_VIDEO, nullptr);

		write(AVMediaType::AVMEDIA_TYPE_AUDIO, nullptr);
	...
	int64_t get_current_duration_tn1e6() {
		int64_t lpts = 0;
		if (has_video_stream()) {
			lpts = av_rescale_q(video_stream.latest_pts, video_stream.enc_ctx->time_base, { 1,AV_TIME_BASE });
		}
		if (has_audio_stream()) {
			auto t = av_rescale_q(audio_stream.latest_pts, audio_stream.enc_ctx->time_base, { 1,AV_TIME_BASE });
			lpts = (std::max)(lpts,t);
		}
		return lpts;
	}
...
public:
	...
	void refresh_vasyn() {
		...
		offset_input_pts_tb1e6 = get_current_duration_tn1e6();//ここの100は適当.
